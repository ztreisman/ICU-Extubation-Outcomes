-- ============================================================================
-- ICU Last Extubations — Revised Pipeline
--
-- Changes from previous version:
--
-- Fix 1: Corrected chartevents nearest-neighbor join (±15 min)
--   The previous ROW_NUMBER() partitioned by (stay_id, charttime) and ordered
--   by ABS(DIFF(charttime, charttime)) — always 0, so ordering was arbitrary.
--   Replaced with LATERAL join that correctly orders by distance to event_time.
--
-- Fix 2: Inferred events never assigned an imputed caregiver_id
--   The previous COALESCE applied chartevents and mode fallbacks to inferred
--   events, assigning plausible-looking but almost certainly wrong caregiver
--   IDs. This was the primary driver of the high-FE artifact: caregivers
--   assigned to many inferred events had inflated FE rates and worse patient
--   survival for reasons unrelated to their performance. Now inferred events
--   retain caregiver_id = NULL throughout. They remain in the dataset for
--   cohort construction and patient-level covariates but are excluded from
--   caregiver-level FE rate calculation.
--
-- Fix 3: Mode fallback narrowed to ±2h window around extubation event
--   Previous fallback used the most frequent caregiver for the entire ICU
--   stay — almost always the primary bedside nurse, not the provider involved
--   in extubation. New fallback uses chartevents within ±2 hours of the
--   extubation event, which is more likely to reflect the active care team.
--
-- Fix 4: procedureevents added as a fallback tier before chartevents
--   Other procedure events documented within ±30 minutes of the extubation
--   are more clinically plausible than chartevents, which can be backdated.
--   New imputation cascade:
--     1. Directly recorded caregiver_id (explicit events only)
--     2. Other procedureevents ±30 min (explicit events only)
--     3. chartevents ±15 min (explicit events only) — bug fixed
--     4. inputevents/outputevents ±5 min (explicit events only)
--     5. Mode caregiver ±2h window (explicit events only)
--     6. NULL for all inferred events
--
-- Fix 5: Aligned failed_extubation_flag and failed_extubation_count
--   failed_extubation_flag in the extubations CTE now requires both the
--   extubation and the reintubation to be explicit events, consistent with
--   reintubation_pairs / reintubation_summary. The commented-out source
--   filters on lines 106-107 are now enforced.
--
-- ============================================================================
...
WITH tube_events AS (
  ...
  FROM (
    ...
    -- Inferred intubation events (caregiver_id always NULL)
    SELECT
      i.subject_id,
      i.hadm_id,
      i.stay_id,
    ...
    WHERE v.ventilation_status = 'InvasiveVent'
      AND DATETIME_DIFF(v.endtime, v.starttime, HOUR) >= 4

    UNION ALL

    -- Inferred extubation events (caregiver_id always NULL)
    SELECT
      i.subject_id,
      i.hadm_id,
      i.stay_id,
  ...
  QUALIFY NOT (
    source = 'inferred' AND (
      (LAG(tube_event) OVER w = tube_event AND LAG(source) OVER w = 'explicit'
        AND TIMESTAMP_DIFF(event_time, LAG(event_time) OVER w, MINUTE) BETWEEN 0 AND 30)
      OR
      (LEAD(tube_event) OVER w = tube_event AND LEAD(source) OVER w = 'explicit'
        AND TIMESTAMP_DIFF(LEAD(event_time) OVER w, event_time, MINUTE) BETWEEN 0 AND 30)
    )
  )
...
-- ── FIX 4: procedureevents fallback (±30 min, explicit events only) ──────────
-- Other procedures documented near the extubation time are more reliable
-- than chartevents because they reflect active clinical actions, not charting.
pe_near_caregiver AS (
  SELECT
    re.stay_id,
    re.event_time,
    pe.caregiver_id,
    ROW_NUMBER() OVER (
      PARTITION BY re.stay_id, re.event_time
      ORDER BY ABS(TIMESTAMP_DIFF(pe.starttime, re.event_time, MINUTE))
    ) AS rn
  FROM ranked_events re
  JOIN `physionet-data.mimiciv_3_1_icu.procedureevents` pe
    ON re.stay_id = pe.stay_id
    AND pe.caregiver_id IS NOT NULL
    AND pe.itemid NOT IN (224385, 227194, 225468, 225477)  -- exclude the extubation events themselves
    AND ABS(TIMESTAMP_DIFF(pe.starttime, re.event_time, MINUTE)) <= 30
  WHERE re.tube_event = 'extubation'
    AND re.source = 'explicit'
),

-- ── FIX 1+3: chartevents nearest-neighbor (±15 min, explicit events only) ───
-- Previous version had a self-referential window ordering bug (always 0).
-- Now uses a correlated approach with correct distance ordering.
ce_near_caregiver AS (
  SELECT
    re.stay_id,
    re.event_time,
    ce.caregiver_id,
    ROW_NUMBER() OVER (
      PARTITION BY re.stay_id, re.event_time
      ORDER BY ABS(TIMESTAMP_DIFF(ce.charttime, re.event_time, MINUTE))
    ) AS rn
  FROM ranked_events re
  JOIN `physionet-data.mimiciv_3_1_icu.chartevents` ce
    ON re.stay_id = ce.stay_id
    AND ce.caregiver_id IS NOT NULL
    AND ABS(TIMESTAMP_DIFF(ce.charttime, re.event_time, MINUTE)) <= 15
  WHERE re.tube_event = 'extubation'
    AND re.source = 'explicit'
),

-- ── FIX 4b: input/output events fallback (±5 min, explicit events only) ─────
-- inputevents and outputevents are recorded at time of clinical action and
-- are harder to backdate than chartevents — more likely to reflect active
-- caregiver presence near the extubation.
io_near_caregiver AS (
  SELECT
    re.stay_id,
    re.event_time,
    ie.caregiver_id,
    ROW_NUMBER() OVER (
      PARTITION BY re.stay_id, re.event_time
      ORDER BY ABS(TIMESTAMP_DIFF(ie.starttime, re.event_time, MINUTE))
    ) AS rn
  FROM ranked_events re
  JOIN `physionet-data.mimiciv_3_1_icu.inputevents` ie
    ON re.stay_id = ie.stay_id
    AND ie.caregiver_id IS NOT NULL
    AND ABS(TIMESTAMP_DIFF(ie.starttime, re.event_time, MINUTE)) <= 5
  WHERE re.tube_event = 'extubation'
    AND re.source = 'explicit'
),

-- ── FIX 3: mode caregiver narrowed to ±2h window (explicit events only) ─────
-- Previous version used the most frequent caregiver for the entire ICU stay,
-- which is almost always the primary bedside nurse — not the provider involved
-- in extubation decisions. ±2h window captures the active care team at the
-- time of extubation while remaining robust to missing data.
cg_fallback_local AS (
  SELECT
    re.stay_id,
    re.event_time,
    ce.caregiver_id
  ...
  JOIN (
    SELECT
      ...
      caregiver_id,
      COUNT(*) OVER (
        PARTITION BY stay_id, caregiver_id
      ) AS n_charts
    FROM `physionet-data.mimiciv_3_1_icu.chartevents`
    WHERE caregiver_id IS NOT NULL
  ) ce
    ON re.stay_id = ce.stay_id
    AND ABS(TIMESTAMP_DIFF(ce.charttime, re.event_time, HOUR)) <= 2
  WHERE re.tube_event = 'extubation'
    AND re.source = 'explicit'
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY re.stay_id, re.event_time
    ORDER BY ce.n_charts DESC
  ) = 1
),
...
extubations AS (
  SELECT
    ...
    -- ── FIX 2: inferred events always get NULL caregiver ─────────────────────
    -- Only explicit events are eligible for caregiver imputation.
    -- Cascade: recorded → procedureevents ±30m → chartevents ±15m →
    --          inputevents ±5m → mode ±2h → NULL
    CASE
      WHEN re.source = 'explicit' THEN
        COALESCE(
          re.caregiver_id,
          pe_near.caregiver_id,
          ce_near.caregiver_id,
          io_near.caregiver_id,
          cg_local.caregiver_id
        )
      ELSE NULL   -- inferred events: no imputation
    END AS caregiver_id,

    -- Track which imputation tier was used (useful for sensitivity analysis)
    CASE
      WHEN re.source = 'inferred'        THEN 'inferred_no_caregiver'
      WHEN re.caregiver_id IS NOT NULL   THEN 'recorded'
      WHEN pe_near.caregiver_id IS NOT NULL THEN 'procedureevents_15m'
      WHEN ce_near.caregiver_id IS NOT NULL THEN 'chartevents_15m'
      WHEN io_near.caregiver_id IS NOT NULL THEN 'inputevents_5m'
      WHEN cg_local.caregiver_id IS NOT NULL THEN 'mode_2h'
      ELSE 'missing'
    END AS caregiver_imputation_source,
    ...
    -- ── FIX 5: failed_extubation_flag now requires both events explicit ───────
    -- Consistent with reintubation_pairs / reintubation_summary which already
    -- required both extubation and reintubation to be explicit.
    IF(
    ...
    -- vent_hours: from prev intubation OR matched ventilation segment
    CASE
  ...
  LEFT JOIN pe_near_caregiver  pe_near  ON re.stay_id = pe_near.stay_id  AND re.event_time = pe_near.event_time  AND pe_near.rn  = 1
  LEFT JOIN ce_near_caregiver  ce_near  ON re.stay_id = ce_near.stay_id  AND re.event_time = ce_near.event_time  AND ce_near.rn  = 1
  LEFT JOIN io_near_caregiver  io_near  ON re.stay_id = io_near.stay_id  AND re.event_time = io_near.event_time  AND io_near.rn  = 1
  LEFT JOIN cg_fallback_local  cg_local ON re.stay_id = cg_local.stay_id AND re.event_time = cg_local.event_time
  WHERE re.tube_event = 'extubation'
),
...
ranked_intubations AS (
  ...
  LEFT JOIN classified_intubations ci
    ON i.subject_id = ci.subject_id
    AND i.hadm_id = ci.hadm_id
    AND i.event_time = ci.event_time
  WHERE vent_hours > 0
),
...
-- caregiver_stats: computed only from explicit events with non-NULL caregiver
-- This ensures inferred events do not inflate FE rates for any caregiver
caregiver_stats AS (
  SELECT
    caregiver_id,
  ...
  FROM extubations
  WHERE caregiver_id IS NOT NULL
    AND source = 'explicit'  -- explicit events only for caregiver stats
...
overall_fe_rate_cte AS (
  ...
  FROM extubations
  WHERE source = 'explicit'  -- consistent with caregiver_stats
),
...
all_ccsr_codes AS (
  SELECT
    d.hadm_id,
    ARRAY_AGG(DISTINCT c.ccsr_category_1 IGNORE NULLS ORDER BY c.ccsr_category_1) AS ccsr_codes,
    ARRAY_AGG(DISTINCT c.ccsr_label_1    IGNORE NULLS ORDER BY c.ccsr_label_1)    AS ccsr_labels
  FROM `physionet-data.mimiciv_3_1_hosp.diagnoses_icd` d
  JOIN `mythical-legend-456217-e9.Failed_Extubation_Rate.ccsr_mappings` c
    ON d.icd_version = 10
   AND REPLACE(d.icd_code, '.', '') = c.icd10cm_code
  WHERE d.seq_num <= 10
  GROUP BY d.hadm_id
...
final AS (
  ...
  LEFT JOIN reintubation_summary r              ON i.subject_id = r.subject_id
